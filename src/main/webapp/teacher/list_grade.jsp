			...
			<div id="top_content"> 
				...
				<div id="content">
					...
					<table class="table">
						...
						<c:forEach items="${one_grade }" var="og">
						<tr class="row1">
							<td>${og.key.snumber }</td>
							<td>${og.key.sname }</td>
							<td>${og.value.course }</td>
							<td>${og.value.grade }</td>
							<td><a href="<%=request.getServletContext().getContextPath()%>/updategrade?snumber=${og.key.snumber}&course=${og.value.course}">修改或添加</a></td>
						<tr>
						</c:forEach>
					</table>
				</div>
